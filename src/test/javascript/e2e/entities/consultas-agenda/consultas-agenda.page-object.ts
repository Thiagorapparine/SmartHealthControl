...
export class ConsultasAgendaComponentsPage {
  createButton = element(by.id('jh-create-entity'));
  deleteButtons = element.all(by.css('jhi-consultas-agenda div table .btn-danger'));
  title = element.all(by.css('jhi-consultas-agenda div h2#page-heading span')).first();
...
export class ConsultasAgendaUpdatePage {
  pageTitle = element(by.id('jhi-consultas-agenda-heading'));
  ...
  usuariosSaudeSelect = element(by.id('field_usuariosSaude'));
  ...
  async usuariosSaudeSelectLastOption(): Promise<void> {
    await this.usuariosSaudeSelect.all(by.tagName('option')).last().click();
  }

  async usuariosSaudeSelectOption(option: string): Promise<void> {
    await this.usuariosSaudeSelect.sendKeys(option);
  }

  getUsuariosSaudeSelect(): ElementFinder {
    return this.usuariosSaudeSelect;
  }

  async getUsuariosSaudeSelectedOption(): Promise<string> {
    return await this.usuariosSaudeSelect.element(by.css('option:checked')).getText();
  }
...
export class ConsultasAgendaDeleteDialog {
  private dialogTitle = element(by.id('jhi-delete-consultasAgenda-heading'));
  private confirmButton = element(by.id('jhi-confirm-delete-consultasAgenda'));
